...
/* server name */
//var rtccopy_server = "ws:localhost:8002";
var rtccopy_server = "ws:localhost:8002"; 
...
function init() {
  ...
  if (room != 0) {
	  ...
	  rtc.on('ready', function(my_socket, usernames) {
		...
		/* getting all the usernames in the room */
		var username_arr = [];//convert to array
		for (var x in usernames) {
			if (x != my_socket) {//don't print yourself
	  ...
	  rtc.on('data stream open', function(id, username) {
		...
		/* log a message (Done in crypto.js if crypto is enabled) */
		if (encryption_type == "NONE") {
